<template>
    <div class="application-config-page">
        <el-card shadow="never" class="list-card">
            <template #header>
                <div class="card-header">
                    <span class="card-title">研究标签管理</span>
                    <el-button type="primary" @click="openCreate">新增研究标签管理</el-button>
                </div>
            </template>
            ...
            <el-table v-loading="loading" :data="tableData" border row-key="tagCode" height="calc(100vh - 330px)">
                <el-table-column prop="tagCode" label="编码" min-width="160" align="center" />
                <el-table-column prop="tagName" label="名称" min-width="160" align="center" />
                <el-table-column prop="tagCategory" label="标签分类" width="130" align="center" />
        ...
        <ResearchTagFormDialog v-model="dialogVisible" :mode="dialogMode" :form-data="currentRow"
            @success="fetchData" />
    </div>
</template>
...
import { queryResearchTagPage, deleteResearchTag } from '/@/api/researchtag/researchTagApi';
...
import ResearchTagFormDialog from './ResearchTagFormDialog.vue';
...
const tableData = ref<ResearchTag[]>([]);
const total = ref(0);
const dialogVisible = ref(false);
const dialogMode = ref<'create' | 'edit'>('create');
const currentRow = ref<ResearchTag>(createDefaultResearchTag());

async function fetchData() {
    loading.value = true;
    try {
        const res = await queryResearchTagPage(queryForm);
...
function openCreate() { 
    ...
    currentRow.value = createDefaultResearchTag(); 
    dialogVisible.value = true; 
}

function openEdit(row: ResearchTag) { 
    dialogMode.value = 'edit'; 
    currentRow.value = { ...row }; 
    dialogVisible.value = true; 
}

async function remove(row: ResearchTag) {
    await ElMessageBox.confirm(`确认删除「${row.tagName}」？`, '提示', { type: 'warning' });
    await deleteResearchTag(row.tagCode);
